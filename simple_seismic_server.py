...
# Pre-compute static responses for performance optimization
# Optimized: Moved static response data to module-level constants to avoid reconstruction
...
class SeismicHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/":
            ...
            resp = STATIC_HEALTH_RESPONSE_BASE.copy()
            resp["timestamp"] = datetime.utcnow().isoformat()
            resp["uptime_seconds"] = int(time.time())
            # Safe copy of mutable nested dict
            resp["services"] = STATIC_HEALTH_RESPONSE_BASE["services"].copy()
            self.send_json(resp)
        elif self.path == "/api/bench/live":
            resp = STATIC_BENCH_RESPONSE_BASE.copy()
            resp["timestamp"] = datetime.utcnow().isoformat()
            # Safe copy of mutable nested dicts
            resp["percentiles"] = STATIC_BENCH_RESPONSE_BASE["percentiles"].copy()
            resp["energy_efficiency"] = STATIC_BENCH_RESPONSE_BASE["energy_efficiency"].copy()
            resp["verification"] = STATIC_BENCH_RESPONSE_BASE["verification"].copy()
            self.send_json(resp)
        elif self.path == "/api/seismic/status":
            resp = STATIC_SEISMIC_RESPONSE_BASE.copy()
            resp["timestamp"] = datetime.utcnow().isoformat()
            # Safe copy of mutable nested dicts
            resp["phases"] = STATIC_SEISMIC_RESPONSE_BASE["phases"].copy()
            resp["landauer_limit"] = STATIC_SEISMIC_RESPONSE_BASE["landauer_limit"].copy()
            self.send_json(resp)
        else:
            self.send_error(404)
    ...
    def send_json(self, data):
        ...
        self.end_headers()
        # Optimized: Compact JSON serialization
