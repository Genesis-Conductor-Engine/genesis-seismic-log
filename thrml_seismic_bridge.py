...
@jax.jit
def _apply_seismic_shock(key, state, stress):
    """
    Phase 2: Seismography (Standalone JIT).
    """
...
@jax.jit
def _verify_crystallization(original_state, re_annealed_state, threshold):
    """
    Phase 3: Crystallization (Standalone JIT).
    """
...
class SeismicWrapper:
    ...
    def apply_seismic_shock(self, key, state):
        ...
        """
        return _apply_seismic_shock(key, state, self.stress)
    ...
    def verify_crystallization(self, original_state, re_annealed_state):
        ...
        """
        return _verify_crystallization(original_state, re_annealed_state, self.threshold)
